...
def calcular_dias_uteis(ano, mes=None):
    ...
    if mes is None:
        opcao = input(
            "Digite 'A' para o ano inteiro ou 'M' para um mês específico: "
        ).upper()

        if opcao == "A":
            for mes in range(1, len(mes)):
...
def calcular_mes(feriados_br, feriados_rs, ano, mes):
    ...
    print(
        "O intervalo original : "
        + str(primeiro_dia.date())
        + " "
        + str(ultimo_dia.date())
    )
        ...
        for day in week:
            if day != 0:
                ...
                if current_date.weekday() >= 5:  # Fim de semana
                    ...
                    current_date in feriados_br
                    or current_date.weekday() == 6
                    or any(
                        date.date() == current_date.date() for date, _ in feriados_rs
                    )
                ):  # Feriado
    ...
    res = sum(
        ...
        if datetime(ano, mes, day).weekday() < 5
        and datetime(ano, mes, day) not in feriados_br
    )
    print("\033[90mTotal de dias úteis no mês : " + str(res))
    print(f"O total de vale transporte neste mês será de R$ {round((4.8 * 4) * res,2)}")
    print(f"O total de vale alimentação neste mês será de R$ {round(23.5 * res,2)+(16.6*res)}")
    print(f"O desconto do VT será de R$ {round(0.06 * ((4.8 * 4) * res),2)}")

    feriados_mes = [
        (date.strftime("%d/%m/%Y"), name)
        for date, name in feriados_br.items()
        if primeiro_dia.date() <= date <= ultimo_dia.date()
    ]
    feriados_mes += [
        (date.strftime("%d/%m/%Y"), name)
        for date, name in feriados_rs
        if primeiro_dia.date() <= date.date() <= ultimo_dia.date()
    ]
